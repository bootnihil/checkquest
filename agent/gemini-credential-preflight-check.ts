...
} from './application/preflight-gemini-credentials';
import {
  CheckQuestError
...
async function main():
  ...
  const unrelatedPermissionError =
    providerError(
      403,
      {
        error: {
  ...
  assert.equal(
    classifyGeminiCredentialRejection(
      Object.assign(
        new Error(
          'Non-JSON authentication failure.'
        ),
        {
          status:
            401
  ...
  let observedCredential:
    string | undefined;
  const accepted =
    await preflightGeminiCredentials(
      {
        geminiApiKey:
          credential
      },
          ...
          async () => {
            throw providerError(
              400,
              {
                error: {
                  message:
                    `Unrelated request failure ${credential}.`,
                  reason:
                    'REQUEST_INVALID'
                }
              }
  ...
  console.log(
    'Gemini credential preflight checks passed.'
  );
}
