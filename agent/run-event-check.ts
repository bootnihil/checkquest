...
async function main(): Promise<void> {
  ...
  emit({
    ...
    startUrl:
      createSafeDisplayUrl(
        'https://user:password@example.test/start?token=secret#private'
      ),
    pageBudget:
      2,
    navigationBudget:
  ...
  const unexpectedAsyncObserver =
    (
      () =>
        Promise.reject(
          new Error(
            'Rejected observer promise must be isolated.'
          )
        )
    ) as unknown as
      RunEventObserver;
  const rejectingEmitter =
    createRunEventEmitter(
      'rejecting-observer',
      unexpectedAsyncObserver
    );
    ...
    await runGeminiRequest(
      ...
      async () => {
        ...
        if (
          ...
          throw Object.assign(
            new Error(
              'raw SDK body with private-token'
            ),
            {
              status:
                503
    ...
    new CheckQuestError(
      ...
      'Safe navigation failure.',
      {
        ...
        navigationStep:
          1,
        requestedUrl:
          'https://user:password@example.test/path?token=private-token#secret',
        finalUrl:
          'https://example.test/final?session=private-token',
        cause:
          new Error(
  ...
  if (
    ...
    assert.equal(
      failedEvent.requestedUrl,
      'https://example.test/path'
    );
    assert.equal(
      failedEvent.finalUrl,
  ...
  console.log(
    'Stage 8D.2 run event contract and observer-isolation checks passed.'
  );
}
...
main().catch(
  (
    ...
    console.error(
      'Stage 8D.2 run event checks failed.',
      error
    );
    process.exitCode =
      1;
