...
async function main():
  ...
  const report =
    buildSiteAgentReport({
      runId:
        'site-agent-report-builder-check',
      startedAt:
        new Date(
      ...
      outcome: {
        type:
          'completed',
        summary:
          'Synthetic report assembly completed.'
      },
      inspectedPages: [],
      canonicalFindings: [],
      passiveSecurity:
  ...
  console.log(
    'Reusable run-site API and pure report assembly checks passed.'
  );
}
...
main().catch(
  (error: unknown) => {
    console.error(
      'Reusable run-site API check failed.',
      error
    );
