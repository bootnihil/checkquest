...
function createContent(
  ...
  return {
    title:
      'Synthetic accessibility fixture',
    headings:
      [],
    bodyText:
      '',
...
function createAccessibilityFinding(
  ...
  return exploratoryQaFindingSchema.parse({
    ...
    confidence:
      'medium',
    title:
      'Synthetic accessibility concern',
    evidence:
      'Synthetic structured control evidence.',
    reasoning:
      'The supplied facts identify a concrete accessibility conflict.',
    suggestedCheck:
      'Review the exact structured control state.',
    evidenceTarget:
      null,
    presentationTarget:
      null,
    ...
    accessibilityDefectBasis: {
      ...
      conflict:
        'The supplied structured properties do not communicate the control consistently.',
      supportingEvidence
    },
...
function isAdmitted(
  ...
  return admitAccessibilityFindings(
    {
      findings:
        [
          finding
      ...
      summary:
        'Synthetic accessibility admission fixture.'
    },
    content
  )
    .findings
...
function main(): void {
  ...
  const neutralFinding =
    createAccessibilityFinding(
      ...
      {
        title:
          'Accessibility semantics in disclosure accessible name',
        evidence:
          'The disclosure has the accessible name "Product".',
        reasoning:
          'This may violate WAI-ARIA and affect screen-reader users.'
      }
    );
  ...
  const incompleteOriginalProductFinding = {
    ...
    structuredIdentity: {
      mechanism:
        'accessibility-semantics' as const,
      observedValue:
        'Product',
      source:
        'accessible-name' as const,
      subject: {
        kind:
  ...
  assert.equal(
    isAdmitted(
      incompleteOriginalProductFinding,
      neutralContent
    ),
    false,
  ...
  const contradictoryStateContent =
    createContent(
      createDisclosure({
        ariaExpanded:
          'false',
  ...
  const contradictoryStateFinding =
    createAccessibilityFinding([
      {
        controlType:
          'disclosure',
  ...
  assert.equal(
    isAdmitted(
      contradictoryStateFinding,
      contradictoryStateContent
    ),
    true,
   ...
   */
  const incompleteGroundedFinding =
    createAccessibilityFinding(
      [],
      {
        evidenceTarget: {
  ...
  const incompleteTabStateFinding =
    createAccessibilityFinding(
      [],
      {
        evidenceTarget: {
          ...
          controlId:
            tokenTab.controlId!,
          accessibleName:
            tokenTab.accessibleName!,
          tabListId:
            tokenTab.tabListId!,
          controlledPanelId:
            tokenTab.ariaControls!,
          desiredState:
            'selected'
        },
        structuredIdentity: {
  ...
  const selectCompatibilityFinding =
    createAccessibilityFinding(
      [],
      {
        title:
          'Select control requires label review',
        evidence:
          'The structured select control has no extracted label.',
        evidenceTarget: {
          kind:
            'select-option',
          controlLabel:
    ...
    admitAccessibilityFindings(
      {
        ...
        summary:
          'Synthetic category-symmetry fixture.'
      },
      selectContent
    )
      .findings
  ...
  const tokenCompatibilityFinding =
    createAccessibilityFinding(
      [],
      {
        title:
          'Unresolved token in disclosure accessible name',
        evidence:
          'The disclosure accessible name is "[#IABV2SETTINGS#]".',
        structuredIdentity: {
          mechanism:
            'unresolved-token',
          observedValue:
  ...
  const reconciledTokenVariants =
    admittedTokenVariants.map(
      (
        ...
        reconcileFindingObservations({
          pageUrl:
            `https://example.com/runtime-token-${index + 1}`,
          pageTitle:
            'Synthetic runtime token fixture',
          ruleFindings:
            [],
          modelFindings:
            [
  ...
  const reconciledTokenFingerprints =
    reconciledTokenVariants.map(
      result =>
        result
  ...
  assert.ok(
    reconciledTokenFingerprints.every(
      fingerprint =>
        fingerprint?.startsWith(
          'identity|unresolved token|'
        )
    ),
    ...
    isAdmitted(
      {
        ...
        accessibilityDefectBasis: {
          expectation:
            'The accessible name should be meaningful.',
          conflict:
            'The model claims the exact accessible name is defective.',
          supportingEvidence: [
            {
              controlType:
                'disclosure',
   ...
   */
  const unstructuredCompatibilityFinding =
    createAccessibilityFinding(
      [],
      {
        title:
          'Legacy unstructured accessibility concern',
        evidence:
          'A supplied page observation warrants human accessibility review.',
        structuredIdentity:
          null,
        evidenceTarget:
          null,
  ...
  const admittedUnstructured =
    admitAccessibilityFindings(
      {
        findings: [
          unstructuredCompatibilityFinding
        ],
        summary:
          'Synthetic unstructured compatibility fixture.'
      },
      neutralContent
    );
  const reconciledUnstructured =
    reconcileFindingObservations({
      pageUrl:
        'https://example.com/synthetic-unstructured',
      pageTitle:
        'Synthetic unstructured fixture',
      ruleFindings:
        [],
      modelFindings:
        admittedUnstructured
          .findings,
      pageContent:
        neutralContent
    });
   ...
   */
  const groundedPrecedenceFinding =
    createAccessibilityFinding(
      [
        {
          controlType:
  ...
  const ruleFinding = {
    ...
    url:
      'https://example.com/synthetic-rule'
  };
  const ruleCompatibilityFinding =
    createAccessibilityFinding(
      [],
      {
        relatedRuleCode:
    ...
    admitAccessibilityFindings(
      {
        ...
        summary:
          'Synthetic exact-rule compatibility fixture.'
      },
      neutralContent
    );
  const reconciledRule =
    reconcileFindingObservations({
      pageUrl:
        ruleFinding.url,
      pageTitle:
    ...
    admitAccessibilityFindings(
      {
        ...
        summary:
          'Synthetic inconclusive accessibility fixture.'
      },
      mismatchedNameContent
    );
  const reconciled =
    reconcileFindingObservations({
      pageUrl:
        'https://example.com/synthetic-accessibility',
      pageTitle:
        'Synthetic accessibility fixture',
      ruleFindings:
        [],
      modelFindings:
        admittedInconclusive
          .findings,
      pageContent:
        mismatchedNameContent
    });
  ...
  assert.match(
    prompt,
    /A neutral property is not a defect basis by itself\./
  );
  assert.match(
    prompt,
    /Generic references to WCAG, WAI-ARIA, assistive technology/
  );
  assert.match(
    prompt,
