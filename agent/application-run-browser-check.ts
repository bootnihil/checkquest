...
async function waitForNoConnections(
  ...
  assert.fail(
    'The browser connection remained open after cancellation.'
  );
}
...
async function main():
  ...
  await listenOnBrowserSafeLoopbackPort(
    server,
    'G1 application boundary fixture'
  );
  ...
  let reportDirectoryPath:
    string | undefined;
  const originalGeminiModel =
    process.env
      .GEMINI_MODEL;
  const processModelSentinel =
    'PROCESS_MODEL_MUST_NOT_CHANGE';
  ...
  try {
      ...
      startCheckQuest({
        ...
        onEvent:
          event => {
            ...
            throw new Error(
              'Subscriber failures must remain isolated.'
            );
          },
        dependencies: {
          analyzePageForQa:
    ...
    const privateFailureEvents:
      RunEvent[] =
        [];
    const privateFailureRun =
      startCheckQuest({
        target:
          `${baseUrl}/private-failure`,
        budgets: {
          pages:
            1,
          navigationSteps:
          ...
          analyzePageForQa:
            async () => {
              throw new CheckQuestError(
                'MODEL',
                `Synthetic model failure: ${credential}`,
                {
                  phase:
                    'synthetic-model',
                  cause:
                    new Error(
                      `Nested SDK cause: ${credential}`
                    )
                }
              );
            }
    ...
    const finalPersistenceEvent =
      persistenceEvents.at(
        -1
      );

    assert.equal(
      finalPersistenceEvent
        ?.type ===
        'run-failed'
        ? finalPersistenceEvent
            .code
        ...
        dependencies: {
          analyzePageForQa:
            async (
                  ...
                  options?.signal
                    ?.addEventListener(
                      ...
                      () => {
                        reject(
                          new Error(
                            'Synthetic cancellation interruption.'
                          )
                        );
                      },
                      {
              ...
              throw new Error(
                'Unreachable analysis completion.'
              );
            }
        }
      });
    ...
    const finalCancellationEvent =
      cancellationEvents.at(
        -1
      );

    assert.equal(
      finalCancellationEvent
        ?.type,
      'run-failed'
    );
    assert.equal(
...
main().catch(
  (
    ...
    console.error(
      'G1 application browser check failed.',
      error
    );
    process.exitCode =
      1;
