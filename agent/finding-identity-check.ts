...
function createIdentity(
  overrides:
    Partial<FindingStructuredIdentity> =
      {}
): FindingStructuredIdentity {
  return {
    mechanism:
...
function main(): void {
      ...
      createFinding(
        {
          ...
          subject: {
            ...identity.subject,
            locator:
              `#page-${pageNumber}-locator`
          }
        },
        `Generated wording variant ${pageNumber}`
      );
    const pageReconciliation =
      reconcileFindingObservations({
        pageUrl:
  ...
  const sharedComponentOtherControl =
    createIdentity({
      subject: {
        ...identity.subject,
        controlId:
  ...
  console.log(
    'Structured finding identity check passed.'
  );
}
