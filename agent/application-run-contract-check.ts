...
async function main():
  ...
  const run =
    startCheckQuest({
      ...
      credentials: {
        geminiApiKey:
          'APPLICATION_KEY_MUST_NOT_LEAK'
      },
      model:
        'application-model-override',
      onEvent:
        event => {
          events.push(
            event
  ...
  const applicationSource =
    await readFile(
      new URL(
        'application/start-checkquest.ts',
        import.meta.url
      ),
      'utf8'
    );
  ...
  for (
    const forbiddenApplicationCoupling of
      [
        '/cli/',
        'process.env',
        'console.',
  ...
  console.log(
    'G1 application run contract and thin CLI checks passed.'
  );
}
...
main().catch(
  (
    ...
    console.error(
      'G1 application run contract check failed.',
      error
    );
    process.exitCode =
      1;
