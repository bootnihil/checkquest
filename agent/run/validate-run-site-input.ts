...
export function validateRunSiteInput(
  input:
    RunSiteValidationInput
  ...
  try {
    ...
    throw configurationError(
      'startUrl must be a valid HTTP or HTTPS URL.',
      error
    );
  }

  if (
    configuredStartUrl.protocol !==
      'http:' &&
    configuredStartUrl.protocol !==
      'https:'
  ) {
    throw configurationError(
      'startUrl must use the HTTP or HTTPS protocol.'
    );
  }
  ...
  if (
    ...
    site.allowedHosts.length ===
      0 ||
    site.allowedHosts.some(
      host =>
        typeof host !==
          'string' ||
        host.trim().length ===
    ...
    throw configurationError(
      'allowedHosts must contain at least one non-empty host.'
    );
  }
  ...
  if (
    ...
    throw configurationError(
      'allowFormSubmission must be a boolean.'
    );
  }
