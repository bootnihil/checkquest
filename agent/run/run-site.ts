...
import { createPageNoveltyState, predictPageIdentity } from '../exploration/page-novelty';
import {
...
async function executeRunSite(
  ...
  try {
    ...
    try {
      ...
      markNavigationUrlAttempted(navigationUrlState, homepageObservation.requestedUrl);
      ...
      const navigationFrontier = createNavigationFrontier();

      const pageNoveltyState = createPageNoveltyState();

      const passiveSecurityRegistry = createPassiveSecurityRegistry();

      const findingLifecycle = createRunFindingLifecycle();
      ...
      const startPagePassiveSecuritySnapshot = await captureMainDocumentSecurity({
        ...
        requestedUrl: homepageObservation.requestedUrl,
        finalUrl: homepageObservation.finalUrl,
      ...
      >({
        startPage: {
          selection: {
            ...
            navigationAudit: {
              ...
              predictedAreaKey: predictPageIdentity(homepageObservation.finalUrl).areaKey,
              predictedRouteFamilyKey: predictPageIdentity(homepageObservation.finalUrl)
                .routeFamilyKey,
        ...
        getNextPage: async completedPages => {
          while (true) {
            throwIfRunCancelled(input.signal, runId, 'navigation-selection');
             ...
             */
            agentSteps = consumeNavigationDecision(site.maxAgentSteps, agentSteps);

            const decision = await (
              input.dependencies?.chooseNavigationLink ?? chooseNavigationLink
            )(site, policyWindow.candidates, navigationBudget, {
              geminiApiKey: input.credentials?.geminiApiKey,
              model: input.model,
              signal: input.signal,
              onEvent: onModelRequestEvent
            });
            ...
            markNavigationUrlAttempted(navigationUrlState, decision.link.url);

            diagnosticsCollector.reset();

            emit({
              type: 'navigation-started',
              message: `Navigation ${agentSteps} started.`,
              navigationStep: agentSteps,
              navigationBudget: site.maxAgentSteps,
              pageNumber: completedPages.length + 1,
              requestedUrl: createSafeDisplayUrl(decision.link.url)
            });

            const { observation: pageObservation, passiveSecuritySnapshot } = await (async () => {
              try {
            ...
            if (navigationResolution.finalUrlAlreadyInspected) {
              emit({
                ...
                message: `Navigation ${agentSteps} resolved to an already-inspected page.`,
                navigationStep: agentSteps,
                navigationBudget: site.maxAgentSteps,
                pageNumber: completedPages.length + 1,
            ...
            emit({
              type: 'navigation-completed',
              message: `Navigation ${agentSteps} completed.`,
              navigationStep: agentSteps,
              navigationBudget: site.maxAgentSteps,
              pageNumber: completedPages.length + 1,
              requestedUrl: createSafeDisplayUrl(navigationResolution.requestedUrl),
              ...
              selection: {
                ...
                navigationAudit: {
                  ...
                  minimumDepthDiscoveredFromUrl:
                    decision.policyCandidate.minimumDepthDiscoveredFromUrl,
        ...
        inspectPage: async (currentPage, pageIndex) => {
          const pageNumber = pageIndex + 1;

          throwIfRunCancelled(input.signal, runId, 'page-inspection');
          ...
          emit({
            ...
            diagnosticCount:
              pageResult.diagnostics.consoleErrors.length +
              pageResult.diagnostics.failedRequests.length
          });
      ...
      const findingMetrics = pageExecutions.reduce<SiteRunFindingMetrics>(
        (totals, execution) => ({
      ...
      const canonicalFindings = getRunFindings(findingLifecycle);
      ...
      try {
        return buildSiteAgentReport({
          ...
          inspectedPages,
          canonicalFindings,
          passiveSecurity: getPassiveSecurityReport(passiveSecurityRegistry),
          findingMetrics
        });
      } catch (error: unknown) {
