...
import { CheckQuestError, type CheckQuestErrorCode } from '../errors/checkquest-error';
...
export type RunEventInput = RunEvent extends infer Event
  ? Event extends RunEvent
    ? Omit<Event, 'timestamp' | 'runId'>
    : never
  : never;

export type RunEventObserver = (event: RunEvent) => void;

export type RunEventEmitter = (event: RunEventInput) => void;

export type RunFailedEventInput = Extract<
  RunEventInput,
  {
    type: 'run-failed';
  }
...
function ignoreObserverRejection(result: unknown): void {
  if (
...
export function deliverRunEvent(observer: RunEventObserver | undefined, event: RunEvent): void {
  ...
  try {
    ...
    ignoreObserverRejection(result);
  } catch {
...
): (event: GeminiRequestEvent) => void {
  return event => {
    switch (event.type) {
      case 'started':
        emit({
          type: 'model-request-started',
          message: `Model request started: ${event.operation}.`,
      ...
      case 'retrying':
        emit({
          type: 'model-request-retrying',
          message: `Model request will retry: ${event.operation}.`,
          operation: event.operation,
          attempt: event.attempt,
          maxAttempts: event.maxAttempts,
      ...
      case 'completed':
        emit({
          type: 'model-request-completed',
          message: `Model request completed: ${event.operation}.`,
          operation: event.operation,
          attempt: event.attempt,
          maxAttempts: event.maxAttempts
        });
...
export function createRunFailureEvent(error: unknown): RunFailedEventInput {
  if (!(error instanceof CheckQuestError)) {
    return {
      type: 'run-failed',
      message: 'An unexpected CheckQuest failure occurred.',
      code: 'INTERNAL'
    };
  }
  ...
  return {
    ...
    ...(error.pageNumber === undefined
      ? {}
      : {
          pageNumber: error.pageNumber
        }),
    ...(error.navigationStep === undefined
      ? {}
      : {
    ...
    ...(error.requestedUrl === undefined
      ? {}
      : {
          requestedUrl: createSafeDisplayUrl(error.requestedUrl)
        }),
    ...(error.finalUrl === undefined
      ? {}
      : {
          finalUrl: createSafeDisplayUrl(error.finalUrl)
        })
  };
}
