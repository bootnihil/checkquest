...
async function main(): Promise<void> {
  console.log('Asking Gemini to handle a non-interactive candidate finding safely...\n');
  ...
  const decision = await planNextAction(
    {
      ...
      pageContent: {
        ...
        headings: ['Clinical AI Platform', 'Built for Enterprise Scale'],
  ...
  console.log('Validated Gemini planner decision:\n');

  console.log(JSON.stringify(decision, null, 2));

  console.log('\nDecision passed plannerDecisionSchema validation.');

  console.log(`Requested safe action: ${decision.action.kind}`);
