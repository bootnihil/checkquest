...
async function main(): Promise<void> {
  ...
  const maxSteps = site.maxExploratoryStepsPerPage;

  console.log('Running constrained real-site autonomous exploration...\n');

  console.log(`Selected site: ${site.name}`);

  console.log(`Start URL: ${site.startUrl}`);

  console.log(`Maximum autonomous steps: ${maxSteps}`);

  console.log(`Form submission allowed by site config: ${site.allowFormSubmission}`);

  if (site.allowFormSubmission !== false) {
    throw new Error(
  ...
  try {
    ...
    try {
      ...
      console.log(`Final URL: ${homepageUrl.toString()}`);
      ...
      const targetLink = navigationLinks.find(
        link => link.url !== site.startUrl && link.url !== homepageUrl.toString()
      );
      ...
      console.log(`HTTP status: ${pageObservation.httpStatus ?? 'unknown'}`);

      console.log(`Final URL: ${pageObservation.finalUrl}`);

      console.log(`Title: ${pageObservation.title}`);
      ...
      const ruleBasedFindings = evaluatePageObservation(pageObservation);

      const initialContent = await extractPageContent(page);

      console.log('\nInitial structured page observation:');

      console.log(`Headings: ${initialContent.headings.length}`);

      console.log(`Buttons: ${initialContent.buttons.length}`);

      console.log(`Text fields: ${initialContent.textFields.length}`);

      console.log(`Select controls: ${initialContent.selects.length}`);

      console.log(`Links: ${initialContent.links.length}`);

      const containsPasswordField = initialContent.textFields.some(
      ...
      if (exploratoryQaAnalysis.findings.length === 0) {
        console.log('No candidate findings were produced.');
      } else {
        for (let index = 0; index < exploratoryQaAnalysis.findings.length; index += 1) {
          const finding = exploratoryQaAnalysis.findings[index];
          ...
          console.log(`Suggested check: ${finding.suggestedCheck}`);

          console.log(`Evidence target: ${JSON.stringify(finding.evidenceTarget)}`);
        }
      }
      ...
      console.log('- maximum 3 planner decisions');

      console.log('- no form submission action exists');

      console.log('- no arbitrary click action exists');

      console.log('- no arbitrary selectors');

      console.log('- no arbitrary JavaScript');

      console.log('- deterministic Playwright executor only');

      console.log(`- candidate findings supplied: ${exploratoryQaAnalysis.findings.length}`);
      ...
      if (!site.allowedHosts.includes(finalUrl.hostname)) {
        throw new Error(
      ...
      const finalContent = await extractPageContent(page);
      ...
      console.log(
        JSON.stringify(
          {
            textFields: finalContent.textFields,

            selects: finalContent.selects
          },
          null,
          2
        )
      ...
      if (result.plannerDecisionCount > maxSteps) {
        throw new Error(`Autonomous loop exceeded hard limit of ${maxSteps} steps.`);
      }
