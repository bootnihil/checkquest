...
import { submitDesktopRun } from './renderer/submit-run';
import { createElapsedStatusText, formatElapsedTime } from './renderer/elapsed-time';
import { isDesktopRunLocallyEligible } from './renderer/form-eligibility';
import { reduceDesktopBudgetProgress } from './renderer/budget-progress';
import { getDesktopCredentialPresentation } from './renderer/credential-presentation';
import { getBudgetStepperAvailability, stepBudgetValue } from './renderer/budget-stepper';
import { calculateFloatingPosition } from './renderer/floating-position';
import { DesktopSessionCredentialStore } from './session-credential';
import { getDesktopInitialWindowBounds } from './window-bounds';
...
async function runSourceBoundaryChecks(): Promise<void> {
  const rendererSource = (
    await Promise.all([
      readFile(new URL('./renderer/app.ts', import.meta.url), 'utf8'),
      readFile(new URL('./renderer/submit-run.ts', import.meta.url), 'utf8'),
      readFile(new URL('./renderer/credential-presentation.ts', import.meta.url), 'utf8'),
      readFile(new URL('./renderer/floating-position.ts', import.meta.url), 'utf8')
    ])
  ).join('\n');
  const preloadSource = await readFile(new URL('./preload.ts', import.meta.url), 'utf8');
  const controllerSource = await readFile(new URL('./run-controller.ts', import.meta.url), 'utf8');
  const sessionCredentialSource = await readFile(
    new URL('./session-credential.ts', import.meta.url),
    'utf8'
  );
  const mainSource = await readFile(new URL('./main.ts', import.meta.url), 'utf8');
  const rendererHtml = await readFile(new URL('./renderer/index.html', import.meta.url), 'utf8');
  const rendererStyles = await readFile(new URL('./renderer/styles.css', import.meta.url), 'utf8');
