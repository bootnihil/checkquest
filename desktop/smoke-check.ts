...
async function main():
  ...
  try {
    ...
    assert.equal(
      await window
        .locator(
          '.info-help, .help-wrap, .field-tooltip, [role="tooltip"]'
        )
        .count(),
      0
    );
    ...
    for (
      ...
      await window
        .locator(
          `[data-destination-button="${destination.name}"]`
        )
        .click();
      assert.equal(
        await window
    ...
    await window
      .locator(
        '[data-destination-button="reports"]'
      )
      .click();
    assert.equal(
      await window
    ...
    console.log(
      'Electron desktop smoke check passed.'
    );
  } finally {
    await electronApplication
      .close();
